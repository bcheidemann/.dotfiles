...
INSTALL_NAME="evil-helix"
INSTALL_LOG_FILE=/tmp/install-evil-helix.log

install() {
    local BASEDIR="$(pwd)"
    echo "Installing $INSTALL_NAME..."
    brew install evil-helix
    echo "Installing $INSTALL_NAME config..."
    mkdir -p "$HOME/.config/helix"
    rm -f "$HOME/.config/helix/config.toml"
    ln -s "$BASEDIR/.config/helix/config.toml" "$HOME/.config/helix/config.toml"
}
